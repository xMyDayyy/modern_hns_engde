...
{
    // Objekte, die nur waehrend der Anfangssequenz existieren
    static const u16 sIntroOnly[] = {
    ...
    // Zustaende, die die Anfangssequenz sonst erst herstellt
    static const u16 sStoryFlags[] = {
    ...
    for (i = 0; i < ARRAY_COUNT(sIntroOnly); i++)
        FlagSet(sIntroOnly[i]);
    for (i = 0; i < ARRAY_COUNT(sStoryFlags); i++)
        FlagSet(sStoryFlags[i]);

    // Welches Haus dem Spieler gehoert, entscheidet ueber die Bewohner.
    // Im eigenen Haus wohnt die eigene Mutter, im anderen die des Rivalen.
